Nachfolgend werden die Kernaspekte der Methodik sowie die Ergebnisse der Funktionsverifikation zusammengefasst und zukünftige Forschungsrichtungen aufgezeigt.

\section{Methodik und Implementierung}
Im Rahmen dieser Arbeit wurde eine kollaborative Aggregationsinfrastruktur für das ReSiLENT-Detektionssystem konzipiert und implementiert. Jede Ladestation generiert standardisierte \textit{Threat Reports} aus vier spezialisierten Detektionseinheiten. Die Verteilung dieser Reports erfolgt über ein privates IPFS-Peer-to-Peer-Netzwerk.

Zur Aggregation variabler Mengen eingehender Threat Reports wurde ein Aggregationsansatz implementiert, dessen Grundprinzip auf Deep-Sets und Attention-Mechanismen basiert. Dadurch kann eine variable Anzahl eingehender Reports in einen Merkmalsvektor fester Größe (\textit{fixed-size representation}) überführt werden.

Die Relevanz einzelner Knoten wird dabei über zwei angepasste Gewichtungsfunktionen modelliert: eine modifizierte \textit{Negative Exponential Utility Function} (CARA) sowie eine angepasste \textit{Sigmoid-Funktion}. Beide Funktionen dienen der dynamischen Gewichtung der eingehenden Reports und begrenzen durch ihr asymptotisches Sättigungsverhalten den Einfluss statistischer Ausreißer.
